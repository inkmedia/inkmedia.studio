...
const services = [
  ...
  },
  {
    title: "UX/UI Design",
    lines: ["UX/UI", "Design"],
    image: "/images/UX-UI-design.webp",
    color: "#f8e8bd",
    href: "/contact",
  ...
  },
  {
    ...
    lines: ["Redesign, Commerce", "& Optimisation"],
    image: "/images/Redesing-commerce-optimisation.webp",
    color: "#cbdcf6",
    href: "/contact",
    label: "Room to grow",
  },
  {
    ...
    lines: ["Digital", "Marketing"],
    image: "/images/Digital-Marketing.webp",
    color: "#e3d2ee",
    href: "/contact",
    label: "Ideas that find an audience",
  },
];
const copies = [-1, 0, 1];
...
export default function ServicesPage() {
  ...
  useEffect(() => {
    ...
    if (!page || document.body.classList.contains("page-is-transitioning"))
      return;
    let finishTimer = 0;
  ...
  useEffect(() => {
    ...
    let pointer: { id: number; x: number; y: number; dragged: boolean } | null =
      null;
    let suppressClick = false;
    const spacing = () => width * (width <= 700 ? 0.6 : 0.24);
    ...
    function paint() {
      ...
      cards.forEach((card) => {
        ...
        const slot =
          ((((index - position + count + 0.5) % count) + count) % count) -
          Math.floor(count / 2) +
          copy * count;
    ...
    function pointerMove(event: PointerEvent) {
      ...
      if (cursor.current) {
        ...
        cursor.current.dataset.visible =
          event.pointerType === "mouse" &&
          !(event.target as HTMLElement).closest("button")
            ? "true"
            : "false";
    ...
    function focus(event: FocusEvent) {
      ...
      const count = services.length;
      const nearest =
        index - 0.5 + Math.round((target - index + 0.5) / count) * count;
      ...
      >
        ...
        <div className="services-intro">
          ...
          <p className="services-scroll-hint">
            <span className="services-desktop-hint">Scroll to see all</span>
            <span className="services-touch-hint">Swipe to explore</span>
          </p>
          <div
            className="services-controls"
            aria-label="Service carousel controls"
          >
            <button
              type="button"
        ...
        <span ref={cursor} className="services-cursor" aria-hidden="true">
          Keep scrolling
        </span>
      </section>
    </main>
